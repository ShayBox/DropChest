    ...
    private void load() {
        ...
        if (yamlFile.exists()) {
            ...
            try {
                ...
                Map<String, Object> chests = yaml.load(reader);
            ...
            if (args.length == 0) {
                ...
                if (args[0].equalsIgnoreCase("add")) {
                    ...
                    if (args.length == 2 && hasPermission(player, "dropchest.radius.set")) {
                        requestedRadius = Integer.valueOf(args[1]);
                    ...
                    if (current > needed)
                        current = 1;
                    if (needed != 1) {
                        sender.sendMessage(ChatColor.BLUE.toString() + "Page " + current + " of " + needed);
                    }
                    ...
                    if (args.length == 3) {
                        ...
                        if (dci != null) {
                            if (ownsChest(dci, sender)) {
                                ...
                                sender.sendMessage("Radius of Chest #" + dci.getId() + " set to " + dci.getRadius());
                                save();
                            } else {
                                sender.sendMessage("That's not your chest.");
                            }
                    ...
                    if (args.length == 3) {
                        ...
                        if (dci != null) {
                            if (ownsChest(dci, sender)) {
                                dci.setDelay(delay);
                                sender.sendMessage(ChatColor.GREEN + "Delay of Chest '" + dci.getName() + "' set to " + dci.getDelay() + " milliseconds.");
                                save();
                            } else {
                                sender.sendMessage("That's not your chest.");
                            }
