...
public class DropChestItem implements Serializable {
    ...
    public Chest getChest() {
        if (getBlock().getState() instanceof Chest) {
            containerBlock = (Chest) getBlock().getState();
    ...
    public Inventory getInventory() {
        ...
        for (BlockFace face : faces) {
            ...
            if (other.getType().equals(Material.CHEST)) {
                if (other.getState() instanceof Chest) {
                    Inventory[] inventories = {getChest().getInventory(), ((Chest) other.getState()).getInventory()};
            ...
            if (splt.length >= 1) {
                ...
                if (data.length >= 6) {
                    ...
                    if (fileVersion.equals("0.3")) {
                        env = Environment.valueOf(data[6]);
                    }
    ...
    public String save() {
        // VERSION!!!! 0.7
        String line = "";
        line = loc.getX() + "," + loc.getY() + "," + loc.getZ() + "," + getRadius() + "," + loc.getWorld().getName();
        line += ",";
        line += "," + loc.getWorld().getEnvironment();
        line += "," + id;
        line += "," + name;
        line += "," + getOwner();
        line += "," + isProtect();
    ...
    public String listString() {
        ...
        ret += " | " + (int) (p * 100) + "%";
        ret += " | " + (getFilter(FilterType.SUCK).size() + getFilter(FilterType.PUSH).size() + getFilter(FilterType.PULL).size()) + " ";
        ret += " | " + getRadius();
        return ret;
    }
